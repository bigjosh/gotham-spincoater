...
View the Pico from above with its USB socket at the top. The middle columns are physical header pin numbers; GP numbers are the names used in MicroPython. This map reflects the project's modified kit, with RGB, buzzer, D1 and D2 GPIO branches isolated as confirmed by the builder on 2026-09-08.
...
**GP12, GP13, GP16 and GP17** are now available after the builder's confirmed isolation of the RGB LED, buzzer and indicator LED branches. The display, touch, buttons and joystick keep their original assignments. `AUX_LINKS_DISCONNECTED = True` unlocks fans #4/#5 while preserving saved selections.
