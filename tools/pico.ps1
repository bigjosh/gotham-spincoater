param(
    [ValidateSet('Info', 'Exec', 'Deploy', 'Launch', 'Monitor')]
    [string]$Action = 'Info',
    [string]$CodeFile,
    [string]$Code,
    [int]$Seconds = 5
    ...
    if ($Action -eq 'Monitor') {
        ...
        if ($Action -eq 'Info') {
            ...
            foreach ($deviceFile in $deviceFiles) {
                ...
                $previous = Invoke-Raw "import os, binascii`nprint(binascii.b2a_base64(open('$remoteName','rb').read()).decode().strip() if '$remoteName' in os.listdir() else 'MISSING')"
                if ($previous -ne 'MISSING') {
                    [IO.File]::WriteAllBytes((Join-Path $backupDir $remoteName), [Convert]::FromBase64String($previous))
                }
                ...
                $remoteHash = Invoke-Raw "import hashlib`nprint(binascii.hexlify(hashlib.sha256(open('$remoteName.upload','rb').read()).digest()).decode())"
...
for _name, _symbol in (('sync_program','sync_program'), ('open_drain_pwm','_open_drain_program')):
...
for _module in ('main','rig','ui','fan','sync_tach','sync_program','periods','open_drain_pwm','display','config'):
...
'@
            ...
            do {
                Start-Sleep -Milliseconds 100
                $message = $connection.ReadExisting()
                if ($message) { Write-Output $message.Replace([string][char]4,'').TrimEnd() }
            } while ((Get-Date) -lt $deadline)
        }
    }
} finally {
