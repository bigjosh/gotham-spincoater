...
This page explains the controller, electrical interface, recipe schema, and development tools. For a new build, start with [the README](README.md#getting-started), [header map](PINOUT.md), and [installation guide](firmware/README.md).
...
See [the wiring tables](README.md#3-wire-the-fans) for each fan's four connections. Tach inputs use the Pico's internal 3.3 V pull-ups. An optional external **4.7 kΩ resistor from a tach GPIO to 3V3(OUT)** can strengthen its pull-up for longer or noisier wiring; never pull a tach input up to 12 V.
...
A 0% command is not a power disconnect or proof of physical stop. A disconnected PWM wire can make a fan run at full speed. Secure the fan and keep spinning parts clear. The direct connection depends on the power sequence above and the powered RP2350 setup; replacing the controller or power arrangement requires reviewing the signal interface. It does not establish compatibility with an RP2040 Pico, driving a fan's PWM input from an ADC-capable GPIO, or a motherboard/hub's higher-voltage tach pull-up. GP28 is intentionally used as a digital **tach input with a 3.3 V pull-up** in this project's pin map.
...
## Maintainer deployment and tests

The tested firmware is **MicroPython v1.29.0 for RPI_PICO2_W**. Use the [portable installation guide](firmware/README.md) for a new board. The Windows workflow below is maintained for the original project board.
...
`tools/test_fan_settings_board.py` checks saved selections, all-off behavior, GPIO locks, the worker heartbeat, and LOW PWM outputs using a separate temporary settings file. It never starts a recipe and preserves the user's `fans.json` and `recipes.json`. Run it through `pico.ps1 -Action Exec -CodeFile tools/test_fan_settings_board.py` after deploying the modules, then reboot into the normal app. Check touch alignment on your assembled display during the first-spin procedure.
